...
const EmptyState = memo(({ input, setInput, handleSubmit, stop, status }: EmptyStateProps) => {
  ...
  const promptSuggestions = [
    { text: "根据我的阅读记忆，推荐 3 本适合我的新书", icon: BookOpen },
    { text: "帮我梳理一下我的阅读偏好和知识结构", icon: Brain },
    { text: "我读过的书里，有哪些共同的核心主题？", icon: ScrollText },
  ] as const;
    ...
    <div className="flex h-full w-full select-none flex-col items-center justify-center overflow-y-auto p-6">
      <div className="mx-auto w-full max-w-3xl space-y-6">
        <div className="flex flex-col items-center gap-2">
          ...
          <h1 className="font-semibold text-3xl text-neutral-900 dark:text-neutral-50">智识助手</h1>
          <p className="text-center text-muted-foreground text-sm max-w-md">
            基于你的阅读记忆——了解你的阅读偏好、推荐新书、发现跨书联系
          </p>
        </div>
        ...
        <div>
          <h2 className="font-medium text-neutral-600 text-sm dark:text-neutral-400">从记忆出发</h2>
...
function ChatPage() {
  ...
  const renderToolContent = () => {
    ...
    if (toolType === TOOL_NAME_MAP.mindmap) {
      return <MindmapViewer mermaidCode={toolDetail.output.results.mermaidCode} />;
    }
