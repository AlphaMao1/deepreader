...
} from "@/services/memory-extraction-pipeline";
...
export function useChatState(options: UseChatStateOptions): UseChatStateReturn {
  ...
  const syncCurrentThread = useCallback(
    (thread: Thread | null) => {
  ...
  useEffect(() => {
    const initializeThread = async () => {
      if (activeBookId && !currentThread && !isInit.current) {
        try {
          ...
          if (latestThread) {
            syncCurrentThread(latestThread);
    ...
    initializeThread();
  }, [activeBookId, currentThread, syncCurrentThread, setMessages]);

  // biome-ignore lint/correctness/useExhaustiveDependencies: <explanation>
  useEffect(() => {
    return () => {
      syncCurrentThread(null);
      setMessages([]);
      setReferences([]);
    };
  ...
  const handleNewThread = useCallback(() => {
    if (currentThreadRef.current?.id) {
      resetExtractionCounter(currentThreadRef.current.id);
    }
    syncCurrentThread(null);
    setMessages([]);
    setDisplayError(null);
