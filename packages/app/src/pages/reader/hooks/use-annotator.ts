  ...
  const addNote = useCallback(async () => {
    ...
    try {
      ...
      if (!annotation) {
        ...
        annotation = await createBookNote({
          bookId,
          type: "annotation",
          cfi,
          style,
      ...
      // 弹出 prompt 让用户写想法
      const existingNote = annotation.note || "";
      const thought = window.prompt("写下你的想法：", existingNote);
      if (thought === null) return; // 用户取消

      // 保存想法到 annotation.note
      const updated = await updateBookNote(annotation.id, {
      ...
      const currentAnnotations = store.getState().config?.booknotes || [];
      const refreshed = currentAnnotations.map((a) => (a.id === annotation!.id ? updated : a));
      const refreshedConfig = store.getState().updateBooknotes(refreshed);
      if (refreshedConfig) {
        await store.getState().saveConfig(refreshedConfig);
      }
      queryClient.invalidateQueries({ queryKey: ["annotations", bookId] });
      toast.success(thought ? "想法已保存" : "想法已清除");
    } catch (error) {
      console.error("写想法失败:", error);
      toast.error("写想法失败");
    }
  }, [selection, config, view, settings, bookId, store, queryClient]);
